...
export const imageClassificationPipelineFormSchema = z.object({
  image: hasFileAPI ? z.instanceof(File) : z.any()});

export async function onImageClassificationPipelineFormSubmit(
    data: z.infer<typeof imageClassificationPipelineFormSchema>
) {
    try {
