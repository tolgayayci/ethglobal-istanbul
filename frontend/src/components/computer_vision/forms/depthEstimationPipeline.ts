...
export const depthEstimationPipelineFormSchema = z.object({
  image: hasFileAPI ? z.instanceof(File) : z.any()});

export async function ondepthEstimationPipelineFormSubmit(
    data: z.infer<typeof depthEstimationPipelineFormSchema>
) {
    try {
      const resultJson = jsonGenerator(TaskTypes.DEPTH_ESTIMATION, [
